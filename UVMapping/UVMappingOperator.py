...
class UVTExture_OT_UvMappingCalculateProjectionValue(bpy.types.Operator):
    ...
    def execute(self, context):   
      ...
      uvMappingObjectName = scene.uv_texture_settings[scene.layer_choose_index].bakeObjName

      backGroundObjName = scene.uv_texture_settings[scene.layer_choose_index].coverObjName

      # default name will use 0 base object
      if backGroundObjName == "Default" :
        # becaues of layers is a stack list ,
        # we want it to be arranged as layers,
        # so zero elements will be base obj(all other objs covered in this)  
        backGroundObjName = scene.uv_texture_settings[0].bakeObjName

      # uv_texture_settings[layer_choose_index].bakeObjName must be has a name in blender scene
      # it can set in UI 
      if uvMappingObjectName != "Default":

          sampleNums = scene.uv_texture_output_config[list(context.scene.uv_texture_output_config.keys()).count() - 1].mappingSampleNums 
         ...
         self.report({'WARRING'},'bakeObjName, layer_choose_index = <' +  str(scene.layer_choose_index) + '> don\'t set a object name in blender scene')    
         raise RuntimeError('bakeObjName, layer_choose_index = <' +  str(scene.layer_choose_index) + '> don\'t set a object name in blender scene')
